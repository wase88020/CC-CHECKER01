...
r = requests.get('p.webshare.io:9999').text
res = r.partition('\n')[0]
...
async def helpstr(message: types.Message):
    ...
    await message.reply(
        " Simple CC Checker Made By @Fakehecker "
    )
...
async def tv(message: types.Message):
    ...
    await message.answer_chat_action("Wait")
...
<b>CHECKEDBY</b>➟ <a href="tg://user?id={message.from_user.id}">{message.from_user.first_name}</a>
""")
    elif "PASS" in r.text:
        res = r.json()
        await message.reply(f"""
...
async def binio(message: types.Message):
    ...
    if len(BIN) < 6:
        return await message.reply("Maintain!")
    if not BIN:
        return await message.reply("Maintain!.")
